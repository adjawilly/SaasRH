...
export default function Domaines() {
  ...
  const createMutation = useMutation({
    mutationFn: async (libelle: string) => {
      const response = await (api as any).post('/api/parametrage/domaines', { libelle })
      return response.data
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['domaines'] })
      notification.success('Domaine créé', 'Le domaine a été créé avec succès')
      form.reset()
      setShowForm(false)
    },
    onError: (error: any) => {
      notification.error('Erreur', error.response?.data?.message || 'Erreur lors de la création du domaine')
    },
  })

  const updateMutation = useMutation({
    mutationFn: async ({ id, libelle }: { id: string; libelle: string }) => {
      const response = await (api as any).put(`/api/parametrage/domaines/${id}`, { libelle })
      return response.data
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['domaines'] })
      notification.success('Domaine modifié', 'Le domaine a été modifié avec succès')
      setEditingId(null)
      form.reset()
      setShowForm(false)
    },
    onError: (error: any) => {
      notification.error('Erreur', error.response?.data?.message || 'Erreur lors de la modification du domaine')
    },
  })

  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const response = await (api as any).delete(`/api/parametrage/domaines/${id}`)
      return response.data
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['domaines'] })
      notification.success('Domaine supprimé', 'Le domaine a été supprimé avec succès')
    },
    onError: (error: any) => {
      notification.error('Erreur', error.response?.data?.message || 'Erreur lors de la suppression du domaine')
    },
  })

  const handleEdit = (domaine: any) => {
    setEditingId(domaine.id)
    form.setValue('libelle', domaine.libelle)
    setShowForm(true)
  }
    ...
    <div className="space-y-4 md:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          >
            <Briefcase size={22} style={{ color: '#2F5FD7' }} />
            <span>Domaines</span>
          </h2>
          <p className="text-xs md:text-sm text-gray-600 mt-1">Gérez les domaines d'activité disponibles</p>
        </div>
        {!showForm && (
          <button
            onClick={() => {
        ...
        <div className="card border-2 border-blue-200 bg-blue-50/30">
          {domaines && domaines.length > 0 ? (
                  ...
                  {domaines.map((dom: any) => (
                    <tr key={dom.id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-4 py-3 whitespace-nowrap">
                        <span className="text-sm font-medium text-gray-900">{dom.libelle}</span>
                      </td>
                      <td className="px-4 py-3 whitespace-nowrap text-right text-sm font-medium">
                        <div className="flex justify-end space-x-2">
                          <button
                            onClick={() => handleEdit(dom)}
            ...
            <div className="text-center py-8 text-gray-500">
              ...
              <p className="text-sm">Aucun domaine disponible</p>
            </div>
          )}
        </div>
      )}
