...
export default function Profils() {
  ...
  const { data: profils, isLoading } = useQuery({
    queryKey: ['profils'],
    queryFn: async () => {
      const response = await (api as any).get('/api/parametrage/profils')
  ...
  const createMutation = useMutation({
    mutationFn: async (libelle: string) => {
      const response = await (api as any).post('/api/parametrage/profils', { libelle })
      return response.data
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['profils'] })
      notification.success('Profil créé', 'Le profil utilisateur a été créé avec succès')
      form.reset()
      setShowForm(false)
    },
    onError: (error: any) => {
      notification.error('Erreur', error.response?.data?.message || 'Erreur lors de la création du profil')
    },
  })

  const updateMutation = useMutation({
    mutationFn: async ({ id, libelle }: { id: string; libelle: string }) => {
      const response = await (api as any).put(`/api/parametrage/profils/${id}`, { libelle })
      return response.data
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['profils'] })
      notification.success('Profil modifié', 'Le profil utilisateur a été modifié avec succès')
      setEditingId(null)
      form.reset()
      setShowForm(false)
    },
    onError: (error: any) => {
      notification.error('Erreur', error.response?.data?.message || 'Erreur lors de la modification du profil')
    },
  })

  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const response = await (api as any).delete(`/api/parametrage/profils/${id}`)
      return response.data
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['profils'] })
      notification.success('Profil supprimé', 'Le profil utilisateur a été supprimé avec succès')
    },
    onError: (error: any) => {
      notification.error('Erreur', error.response?.data?.message || 'Erreur lors de la suppression du profil')
    },
  })

  const handleEdit = (profil: any) => {
    setEditingId(profil.id)
    form.setValue('libelle', profil.libelle)
    setShowForm(true)
  }
    ...
    <div className="space-y-4 md:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          >
            <Settings size={22} style={{ color: '#2F5FD7' }} />
            <span>Profils utilisateurs</span>
          </h2>
          <p className="text-xs md:text-sm text-gray-600 mt-1">Gérez les profils utilisateurs disponibles</p>
        </div>
        {!showForm && (
          <button
            onClick={() => {
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            <div>
              ...
              <input
                ...
                placeholder="Ex: administrateur, compte_rh..."
              />
            </div>
        ...
        <div className="card border-2 border-blue-200 bg-blue-50/30">
          {profils && profils.length > 0 ? (
            ...
            <div className="text-center py-8 text-gray-500">
              ...
              <p className="text-sm">Aucun profil utilisateur disponible</p>
            </div>
          )}
        </div>
      )}
