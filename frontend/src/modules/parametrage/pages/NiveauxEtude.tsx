...
import api from '../../../api'
import { GraduationCap, Plus, Edit2, Trash2, X, Save } from 'lucide-react'
...
export default function NiveauxEtude() {
  ...
  const createMutation = useMutation({
    mutationFn: async (libelle: string) => {
      const response = await (api as any).post('/api/parametrage/niveaux-etude', { libelle })
      return response.data
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['niveaux-etude'] })
      notification.success('Niveau créé', 'Le niveau d\'étude a été créé avec succès')
      form.reset()
      setShowForm(false)
    },
    onError: (error: any) => {
      notification.error('Erreur', error.response?.data?.message || 'Erreur lors de la création du niveau')
    },
  })

  const updateMutation = useMutation({
    mutationFn: async ({ id, libelle }: { id: string; libelle: string }) => {
      const response = await (api as any).put(`/api/parametrage/niveaux-etude/${id}`, { libelle })
      return response.data
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['niveaux-etude'] })
      notification.success('Niveau modifié', 'Le niveau d\'étude a été modifié avec succès')
      setEditingId(null)
      form.reset()
      setShowForm(false)
    },
    onError: (error: any) => {
      notification.error('Erreur', error.response?.data?.message || 'Erreur lors de la modification du niveau')
    },
  })

  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const response = await (api as any).delete(`/api/parametrage/niveaux-etude/${id}`)
      return response.data
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['niveaux-etude'] })
      notification.success('Niveau supprimé', 'Le niveau d\'étude a été supprimé avec succès')
    },
    onError: (error: any) => {
      notification.error('Erreur', error.response?.data?.message || 'Erreur lors de la suppression du niveau')
    },
  })

  const handleEdit = (niveau: any) => {
    setEditingId(niveau.id)
    form.setValue('libelle', niveau.libelle)
    setShowForm(true)
  }
    ...
    <div className="space-y-4 md:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-xs md:text-sm text-gray-600 mt-1">Gérez les niveaux d'étude disponibles</p>
        </div>
        {!showForm && (
          <button
            onClick={() => {
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            <div>
              ...
              <input
                ...
                placeholder="Ex: Bac, Bac+2, Bac+3..."
              />
            </div>
        ...
        <div className="card border-2 border-blue-200 bg-blue-50/30">
          {niveaux && niveaux.length > 0 ? (
                  ...
                  {niveaux.map((n: any) => (
                    <tr key={n.id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-4 py-3 whitespace-nowrap">
                        <span className="text-sm font-medium text-gray-900">{n.libelle}</span>
                      </td>
            ...
            <div className="text-center py-8 text-gray-500">
              ...
              <p className="text-sm">Aucun niveau d'étude disponible</p>
            </div>
          )}
        </div>
      )}
